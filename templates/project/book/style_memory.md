...
- 高频使用"不是X而是Y / 不是X，是Y"来解释差异。默认禁用；确需使用时每章最多 1 次，且只能用于真正关键的概念反转或人物认知切分，并在 review 中说明不可替代性。
...
- "不是X而是Y / 不是X，是Y"句式限额：默认禁用；确需使用时每章最多 1 次。写之前必须有明确理由；非必要处改为肯定句、动作、对话、物件或后果。
...
样本文风权威位置：`style/samples.md`。

如果 `style/samples.md` 非空：
- 写作前必须从中提取 3-5 条本章可执行的正向文风锚点，写入 `writing_packet.md` 的 Writing Card。
- 冷读和 review 必须检查样本文风对齐：句子节奏、段落手感、描写温度、对话语感、情绪处理。
- 只迁移语言方法，不迁移样本人名、地名、剧情、专有设定、标志性桥段。

如果 `style/samples.md` 为空：按本文件和 `style/rewrite_rules.md` 执行。
