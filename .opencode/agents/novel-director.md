...
- 当 `rolling_plan.yml` 接近耗尽、世界变窄、主角成长过密、支线断供或用户要求开发后续剧情时，调用 `novel-architect` 生成 story development pack。
- architect 是战略建议 subagent，不是合并入口。director 必须审核 development pack 后，才可把接受内容写入 `story_architecture.yml`、`thread_board.yml`、`entities/`、`ledgers/`、`future_backlog.yml` 或 `rolling_plan.yml`。
...
- 如果 `rolling_plan.yml` 只剩 0-3 章、未来窗口明显单薄、连续任务推进、世界缩小或下一轮需要新剧情燃料：先运行 validator/state drift 检查，编译 `planning/context_packs/architect_context_pack_XXX.md`，调用 `novel-architect` 生成 `planning/development_packs/dev_XXX.md`，审核并合并接受内容，再刷新 rolling_plan。
...
只用于写作前的全局故事开发和世界运营设计。调用时优先提供：

- `planning/context_packs/architect_context_pack_XXX.md`
- `planning/story_architecture.yml`
- `planning/thread_board.yml`
- `book/constitution.md` 和 `book/longform_blueprint.yml` 的相关摘要
- 当前卷目标和状态
- `active_flow.yml`
- `rolling_plan.yml`
- 近 6-9 章 summary
- 与本次开发相关的 entities/ledgers 摘要

architect 可以写 `planning/development_packs/dev_XXX.md`，并提出对 `story_architecture.yml`、`thread_board.yml`、`future_backlog.yml`、`rolling_plan.yml`、`entities/`、`ledgers/` 的更新建议；不能写正文，不能合并 canon/current state，不能直接修改 protected files。

如果 architect 标记 `needs_novel_change`、状态漂移、信息释放边界不清或要求修改 protected files，director 必须停止写作准备并处理该问题。
...
必须要求 cold-reader 单独检查"叙述自然性与断句流畅性"：大量莫名短句、机械句号、破折号堆叠、碎片化断句或同类顿挫连续重复，若已经影响阅读顺滑度，必须要求 writer 局部修稿，严重时不得进入 final。
