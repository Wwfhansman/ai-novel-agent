...
- 人物是否有身体、情绪、习惯和关系温度；
- 叙述是否自然顺滑，是否存在大量莫名短句、机械句号、破折号堆叠、碎片化断句、同类节奏连续重复，导致读者被句读绊住；
...
- 判断短句、句号、破折号、省略号和段落切分是否服务情绪/动作/喜剧节奏。
- 如果出现"莫名短句连打"、"每句都断得像提纲"、"破折号替代正常语法连接"、"连续段落同一种顿挫"、"为了短而短导致阅读卡顿"，必须指出具体位置。
- 结论写：smooth / minor friction / revise required。

## 局部润色建议

- 只给 3-5 条读者视角建议。
- 每条说明问题类型：断句僵硬 / 短句堆叠 / 破折号过密 / 描写不自然 / 对话像信息交换 / 转场突兀 / 句式重复。
...
- 如果大量短句、句号、破折号或碎片化断句已经影响阅读流畅性，输出 `revise required`；如果只是局部卡顿，可输出 `pass` 但必须列出具体润色位置。
