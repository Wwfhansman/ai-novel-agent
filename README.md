...
### 写作前必须生成 Context Pack 和 Writing Packet

agent 正式写正文之前，必须先生成可复查的 round context pack，再为每章生成 `writing_packet.md`。round context pack 负责批次级共识；writing packet 合并证据包和正文抬头纸，不再拆成旧的 `brief.md`、`context_pack.md`、`prompt.md` 三件套。
...
- 信息可见性
- 世界压力
- `Writing Card`：分离 Chapter Design 和 Writing Execution
- `Pre-Draft Self Check`
- 写完后必须更新哪些文件

这让写作输入变得可审计、可复现，也方便判断问题到底出在“前文理解”还是“正文生成”。

Context pack / writing packet 不是把项目资料全文复制一遍。正确方式是：
...
chapter writing_packet: 1000-2500 中文字
```
...
- `novel-archivist`：严格记忆档案员 subagent，可选生成 summary/canon 草案。
...
OpenCode 会按 `opencode.json` 默认进入 `novel-director`。你输入写作提示词，例如：
...
- `novel-planner`：根据 round context pack、active_flow 和 rolling_plan 生成每章 `writing_packet.md` 草案。
- `novel-writer`：根据 `writing_packet.md`、上一章结尾和样本文风锚点写 `draft.txt` / 修订 `final.txt`。
- `novel-cold-reader`：生成 `reader_pass.md`，只看文笔、节奏、人物体温和读者体验。
- `novel-qa`：检查 validator、TXT/YAML 格式和质量门漏项。
- `novel-archivist`：在 final 确认后生成 `memory_update_plan.md` 和记忆更新草案；只有明确指定 `mode: canon_draft` / `mode: full_chapter_memory_draft` 时，才生成 `summary.yml` / `canon_delta.yml` 草案。

当前模板把机械检查类 subagent 配成 DeepSeek 轻量模型：

```yaml
model: deepseek/deepseek-v4-flash
```

正文和规划相关 subagent 配成更强模型：

```yaml
model: opencode-go/deepseek-v4-pro
```

如果你使用其他模型提供商，或者你的 OpenCode `/models` 里模型 ID 不同，只需要改这些文件的 `model:` 行：
...
例如你可以换成自己已连接 provider 下的模型。建议只把冷读和 QA 放到轻量模型；planner、writer、正文 final、重大剧情方向、active_flow/rolling_plan 重构和 canon 最终合并仍应由强模型或人类确认。
...
- 先编译 round context pack，再为本批次每章生成 `writing_packet.md`；不要再生成旧的 brief/context_pack/prompt 三件套。
- 每轮必须全文读取 planning/rolling_plan.yml，但 context pack 只摘录本批次、相邻章节和影响本批次的后续约束。
- round context pack 控制在 3000-5000 中文字；chapter writing packet 控制在 1000-2500 中文字，Writing Card 保持紧凑。
- 人物、物品、伏笔、债务、knowledge_state、world_state 按本章涉及对象定向读取，不要整份复述。
- 每章都必须从上一章的 actual_handoff 或 active_flow 的外部压力中长出来。
- 每章都必须在 `writing_packet.md` 中写清楚：
  - Chapter Design / Writing Execution：设计面写变化目标，执行面写 opening_sensory、scene_moments、ending_gesture。
  - time_span、ending_type、position_in_flow：避免连续"一天一章"和"决定下一步"收尾。
  - chapter_function 和 pressure_curve.position_in_flow：本章功能与所在大弧位置。
  - reader_question_flow：本章承接和留下的读者疑问。
  - core_advance：唯一核心推进。
  - must_not_complete：本章刻意不完成的收编、摊牌、解释、证据引爆或关系变化。
  - information_release：本章只释放 1-2 个核心新变量；每个核心变量必须写 enters_via，其余延后。
  - side_yield：世界/系统质感、关系变化、资源/地位/账本变化或可复用伏笔。
  - 叙事织入：至少 3 个不直接解决任务、但让人物、场景、世界或系统自然生长的织入节拍。
  - density_control：限制主要任务数和新信息节点数。
- 每章写 draft 前必须完成 `Pre-Draft Self Check`；如果信息只能靠主角脑内总结进入，先重设场景。
- 默认由 `novel-writer` 写 `draft.txt`、根据 `reader_pass.md` 修稿、并在质量门通过后写 `final.txt`；writer 不改 summary、canon、entities、ledgers 或 planning。
...
- final 前运行：`python scripts/check_not_but.py projects/my-novel --chapters chXXX chYYY chZZZ --files draft.txt`。not-but、三连否定、元叙述、箭头/编号式认知总结都必须先改 draft。
...
- 确认本轮已经全文读取 planning/rolling_plan.yml；本章 writing packet 只摘本章条目、前后相邻条目和必要后续约束。
- 读取上一章 final.txt、summary.yml、canon_delta.yml。
- 按需读取本章涉及的人物、物品、伏笔、债务和信息可见性条目。
- 如果上一章有 actual_handoff，本章开头必须承接；如果不承接，必须在 writing_packet.md 和 review.md 说明原因。
- 写作时不要机械翻译章纲，不要生成固定三段式或任务报告式正文。
- 本章只能有一个核心外部推进。写作前必须列出 time_span、ending_type、position_in_flow、chapter_function、reader_question_flow、core_advance、must_not_complete、information_release.enters_via、side_yield、叙事织入、density_control、scene_moments、ending_gesture。
...
→ 规划未来 9-15 章
→ 写一轮 3 章
→ 生成 round context pack 和每章 writing_packet
→ 写 draft/final 正文
...
默认生产批次是一轮三章，但三章不是叙事单位。每一章都必须从上一章外部交接里长出来，并把具体压力交给下一章；每一章都必须独立生成 writing packet 并独立更新记忆。
...
`rolling_plan.yml`. Round context packs and chapter writing packets must include a
