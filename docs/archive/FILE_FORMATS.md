...
chapters/ch001/
  ...
  draft.txt
  reader_pass.md
  reader_recheck.md        # 仅 reader_pass 曾要求 revise 时需要
  final.txt
...
### 5.1 writing_packet.md

新项目 v2 的唯一写作前输入。它合并旧 `brief.md`、`context_pack.md`、`prompt.md`，但不能削弱质量门。
...
这些 writer 专用字段不是装饰项。`voice_examples` 负责人物语感，`foreshadowing_weight` 负责伏笔和道具的分量感，`relationship_temperature` 负责关系温度，`body_scene_texture` 负责身体与场景质感，`dialogue_mode` 负责对话是试探、权谋、亲密、冲突还是日常。缺少这些信息时，writer 容易把正文写成对章纲的机械翻译。
...
`writing_packet.md` 和 `review.md` 的关键标题是机器校验接口，不只是排版。不要把 `Source References`、`Reader Reward Check`、`Sample Alignment Check`、`TXT 格式检查`、`记忆更新检查` 等固定标题改成自由命名；需要中文解释时写在标题下方。

如果 `style/samples.md` 非空，`Writing Card` 必须包含 3-5 条正向样本文风锚点，覆盖句子节奏、段落手感、描写温度、对话语感或情绪处理。样本只提供语言方法，不允许迁移样本人名、地名、剧情、专有设定或标志性桥段。

连续 draft 模式下，允许在下一章 `writing_packet.md` 的写作自由笔记中临时记录 3-5 行 `draft_handoff_note`。它只用于保持 prose 热态承接，不是正史，不替代 `summary.yml` / `canon_delta.yml` 的 `actual_handoff`。
...
- 最值得保留的一段。如果没有，写"没有"。
- 最生硬的 3 处。
- 叙述自然性与断句流畅性：检查短句、句号、破折号、省略号和段落切分是否服务情绪/动作/喜剧节奏；大量莫名短句、机械句号、破折号堆叠或碎片化断句影响阅读时，必须明确指出具体位置。
- 局部润色建议：3-5 条，只看断句、短句堆叠、破折号过密、描写自然度、对话呼吸、转场和句式重复；可以给短句级改法，不整段代写，不改变剧情事实。
- 必须重写的 1-2 个局部。
- 是否允许进入 final：`pass` / `revise required`。

如果 `reader_pass.md` 输出 `revise required`，找不到值得保留的一段，或认为断句/短句/破折号问题已经影响阅读流畅性，不能写入 `final.txt`。

如果 `reader_pass.md` 曾输出 `revise required`，修稿后必须产出 `reader_recheck.md`。recheck 只检查原硬伤是否解决，不重做全文审稿；verdict 必须是 `pass`。
...
如果由 `novel-archivist` 生成，必须使用 diff-only 格式，目标 50 行以内；不确定项必须写入 `Manual Review`，不得静默猜测。

`novel-archivist` 默认只能直接把草案写入 `chapters/chXXX/memory_update_plan.md`。只有 director 明确指定 `mode: canon_draft` 或 `mode: full_chapter_memory_draft` 时，它才可以生成 `summary.yml` / `canon_delta.yml` 草案。它不能直接修改 `entities/`、`ledgers/`、`planning/` 或任何受保护文件。
...
每条候选更新必须有 `target`、`operation`、`value`、`evidence` 和 `confidence`。禁止复述 summary，禁止输出完整 YAML 草案。

### 5.8 merge_preview.yml

工程合并前必须先生成 `planning/merge_previews/round_XXX.yml`。它由 `scripts/round_state_merge.py preview` 生成，用于预览本轮会写入 `entities/`、`ledgers/`、`planning/` 的结构化操作。

`apply` 只允许自动应用 `confidence: high`、`status: pending` 且无冲突的操作。不确定、冲突、受保护文件、终局秘密相关更新必须留在 `manual_review`。

`canon_delta.yml` 的 `state_sync.status: needs_director_review` 是合并前待审标记。章节进入 post-merge QA 前，director 必须把每个待审目标闭环为 `merged` / `updated` / `synced`，或在确实无实质变化时改为 `n/a`。不得一边保留 `needs_director_review`，一边在 `review.md` 中声称 state_sync 已全部合并。
...
`rolling_plan.yml` 只保留未来窗口。新项目建议保留 6-10 章详细窗口，当前活跃窗口最多 15 章条目。
...
编剧层规则以 [STORY_ARCHITECTURE.md](STORY_ARCHITECTURE.md) 为准。

`planning/story_architecture.yml` 是当前卷/当前阶段的编剧控制台。它把 `book/longform_blueprint.yml` 的全书级约束操作化为当前卷目标、节奏预算、成长预算、信息释放计划和世界扩张执行方案。它不是受保护蓝图本身，不能覆盖 `longform_blueprint.yml`。

`planning/thread_board.yml` 是活跃支线和 off-screen 行动板。它记录编剧视角的支线生命周期、冲突网络和世界独立行动计划。它不是 `ledgers/world_state.yml` 的替代品：`world_state` 是当前世界压力快照，`thread_board` 是未来行动调度。

`planning/development_packs/dev_XXX.md` 是每次 `novel-architect` 运行后的开发快照和建议记录。旧 pack 保留用于审计和冷启动回溯，但 architect 默认不读取旧 pack；它读取 `story_architecture.yml` 和 `thread_board.yml` 的浓缩现状。

`planning/completed_threads_log.yml` 归档已经进入 `aftermath` 并退出活跃调度的支线。`thread_board.yml` 只保留活跃线程。

编剧层文件与现有账本的边界：

- `knowledge_state.yml` 记录已发生的信息分布；`story_architecture.yml` 记录未来信息释放计划。
- `narrative_debts.yml` 和 `foreshadowing.yml` 记录读者期待和伏笔证据；`thread_board.yml` 记录编剧调度中的线程状态。
- `world_state.yml` 记录当前外部压力；`thread_board.offscreen_actions` 记录带时间线的未来行动计划。
- `longform_blueprint.yml` 记录全书级规模递进；`story_architecture.scale_execution` 记录当前卷如何执行这个递进。
